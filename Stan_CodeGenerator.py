...
def required_data_keys(selected_data: list[str]) -> list[str]:
    return unique([*selected_data, *CHI_SQ_DATA_KEYS, "BrRatio2025"])
...
def build_model_block(selected_data: list[str], ff_model: str, ff_nf: int, np_model: str, ub_model: str, fit_functions: FitFunctionSet) -> str:
    modules = data_modules(np_model)
    terms = [modules[key].model_term for key in selected_data if modules[key].model_term]
...
def resolve_output_keys(selected_data: list[str], generated_outputs: Any, np_model: str) -> list[str]:
    modules = data_modules(np_model)
    raw_keys = [modules[key].raw_output_key for key in selected_data if modules[key].raw_output_key]
    if generated_outputs == "Automatic" or generated_outputs is None:
        base = ["VcbFromBr", "RD", *raw_keys]
    else:
        base = list(generated_outputs)
    return unique(["VcbFromBr", *base])


def output_decl(key: str) -> str:
    return {
        "VcbFromBr": "vector[2] rawBrRatio_func;\nvector[2] BrRatio_func;\nvector[2] BrRatio_data;\nvector[2] Vcb_Br;\nreal Vcb_mean;",
        "RD": "vector[2] RD_func;",
        "RawBelleD": "vector[10] rawBelleD_func;",
        "RawBelleDstW": "vector[10] rawBelleDst_w_func;",
        "RawBelleDstCosL": "vector[10] rawBelleDst_cosL_func;",
        "RawBelleDstCosV": "vector[10] rawBelleDst_cosV_func;",
        "RawBelleDstChi": "vector[10] rawBelleDst_chi_func;",
    }[key]
...
def output_body(key: str, fit_functions: FitFunctionSet) -> list[str]:
    ...
    return fit_functions.output_bodies[key]


def full_chi_sq_lines(np_model: str) -> list[str]:
    modules = data_modules(np_model)
    lcsr18_diag = (
        "dot_self((bszLCSR18cent - LCSR18func) ./ sqrt(diagonal(bszLCSR18cov)))"
        if np_model == "T"
        else "dot_self((bszLCSR18centNoTensor - LCSR18func) ./ sqrt(diagonal(bszLCSR18covNoTensor)))"
    )
    return [
        f"chi_sq[1] = {modules['MILC15'].chi_sq_expr};",
        f"chi_sq[2] = {modules['MILC21'].chi_sq_expr};",
        f"chi_sq[3] = {modules['JLQCD23'].chi_sq_expr};",
        f"chi_sq[4] = {modules['HPQCD23'].chi_sq_expr};",
        f"chi_sq[5] = {modules['BelleFITD'].chi_sq_expr};",
        f"chi_sq[6] = {modules['BelleFITDstW'].chi_sq_expr};",
        f"chi_sq[7] = {modules['BelleFITDstCosL'].chi_sq_expr};",
        f"chi_sq[8] = {modules['BelleFITDstCosV'].chi_sq_expr};",
        f"chi_sq[9] = {modules['BelleFITDstChi'].chi_sq_expr};",
        "chi_sq[10] = dot_product(BrRatio2025cent - BrRatio_func, inverse(BrRatio2025cov) * (BrRatio2025cent - BrRatio_func));",
        f"chi_sq[11] = {modules['LCSR18'].chi_sq_expr};",
        f"chi_sq[12] = {modules['LCSR23'].chi_sq_expr};",
        f"chi_sq[13] = {lcsr18_diag};",
        "chi_sq[14] = dot_self((bszLCSR23cent - LCSR23func) ./ sqrt(diagonal(bszLCSR23cov)));",
...
def build_generated_quantities_block(selected_data: list[str], generated_outputs: Any, np_model: str, fit_functions: FitFunctionSet) -> str:
    ...
    decls.extend(["vector[14] chi_sq;", "real chi_sq_total;"])
    bodies: list[str] = []
    for key in output_keys:
        bodies.extend(output_body(key, fit_functions))
    bodies.extend(full_chi_sq_lines(np_model))
...
def build_stan_code(
    *,
    ...
    known = set(data_modules(np_model))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate Stan fit files without Wolfram.")
